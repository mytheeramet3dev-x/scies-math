//! Benchmark: Matrix multiply — scies-math-th vs naive O(n³)
//!
//! Run with:
...
use scies_math_th::perf::{matmul, matmul_threaded};
...
// ── scies-math-th tiled/Strassen (auto-dispatch) ───────────────────────────

fn bench_scies_matmul(c: &mut Criterion) {
    let mut group = c.benchmark_group("matmul/scies-math-th");
...
fn bench_f32_vs_f64(c: &mut Criterion) {
    ...
    group.bench_function("f32-256-via-Mat", |bench| {
        ...
        bench.iter(|| criterion::black_box(ma.matmul(&mb).unwrap()));
    });
