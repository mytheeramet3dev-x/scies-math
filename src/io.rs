...
impl CsvMatrix {
    ...
    pub fn read<P: AsRef<Path>>(path: P, has_header: bool) -> SciResult<DynamicMatrix> {
        let file = File::open(path).map_err(|_| SciError::InvalidParameter("Could not open file"))?;
        ...
        for (i, line_result) in reader.lines().enumerate() {
            ...
            if cols == 0 {
                ...
                return Err(SciError::InvalidParameter("Inconsistent number of columns in CSV"));
            }
    ...
    pub fn write<P: AsRef<Path>>(path: P, matrix: &DynamicMatrix) -> SciResult<()> {
        let file = File::create(path).map_err(|_| SciError::InvalidParameter("Could not create file"))?;
        let mut writer = BufWriter::new(file);

        for r in 0..matrix.rows() {
            for c in 0..matrix.cols() {
                let val = matrix.get(r, c)?;
                write!(writer, "{}", val).map_err(|_| SciError::InvalidParameter("Error writing to file"))?;
                if c < matrix.cols() - 1 {
                    write!(writer, ",").map_err(|_| SciError::InvalidParameter("Error writing to file"))?;
                }
            }
...
impl MatrixMarket {
    ...
    pub fn read<P: AsRef<Path>>(path: P) -> SciResult<DynamicMatrix> {
        ...
        if dims.len() < 2 {
            return Err(SciError::InvalidParameter("Invalid dimensions in MatrixMarket file"));
        }

        let rows: usize = dims[0].parse().map_err(|_| SciError::InvalidParameter("Invalid row count"))?;
        let cols: usize = dims[1].parse().map_err(|_| SciError::InvalidParameter("Invalid col count"))?;
    ...
    pub fn write<P: AsRef<Path>>(path: P, matrix: &DynamicMatrix) -> SciResult<()> {
        ...
        writeln!(writer, "% Generated by scies-math").unwrap();
