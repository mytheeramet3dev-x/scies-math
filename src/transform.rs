...
impl Quaternion {
    ...
    pub fn from_axis_angle(axis: [f64; 3], angle_rad: f64) -> SciResult<Self> {
        ...
        if n < f64::EPSILON {
            return Err(SciError::InvalidParameter("zero axis"));
        }
...
impl Rotation3 {
    ...
    pub fn from_axis_angle(axis: [f64; 3], angle_rad: f64) -> SciResult<Self> {
        Ok(Self(
            Quaternion::from_axis_angle(axis, angle_rad)?.normalize()?,
        ))
    }

    pub fn from_euler_xyz(roll: f64, pitch: f64, yaw: f64) -> SciResult<Self> {
        Ok(Self(
            Quaternion::from_euler_xyz(roll, pitch, yaw).normalize()?,
        ))
    }
    ...
    pub fn to_matrix(&self) -> SMatrix<f64, 3, 3> {
...
impl Isometry3 {
    ...
    pub fn from_parts(rot: SMatrix<f64, 3, 3>, t: [f64; 3]) -> SciResult<Self> {
        Ok(Self {
            rotation: Rotation3::from_matrix(&rot)?,
            translation: t,
        })
    }
...
impl Similarity3 {
    ...
    pub fn new(iso: Isometry3, scale: f64) -> SciResult<Self> {
        if scale <= 0.0 {
            return Err(SciError::InvalidParameter("scale must be > 0"));
        }
        Ok(Self {
            isometry: iso,
            scale,
...
pub fn perspective(
    ...
    near: f64,
    far: f64,
) -> SciResult<SMatrix<f64, 4, 4>> {
    if near <= 0.0 || far <= near {
        return Err(SciError::InvalidParameter("0 < near < far"));
    }
...
fn normalize3(v: [f64; 3]) -> SciResult<[f64; 3]> {
    let n = dot3(v, v).sqrt();
    if n < f64::EPSILON {
        return Err(SciError::InvalidParameter("zero vector"));
    }
